...
- ```
  ...
  ```
   ...
   — Platform-specific object of type
  ...
  , having at least the following fields:
...
- ```
  ...
  A module that implements a load-balancing algorithm must set these methods and initialize private
  ...
  was not initialized during configuration parsing,
...
- ```
  ...
  ```
   ...
   — The method called when the upstream module is ready to pass a request to an upstream server and needs to know its address. The method has to fill the
...
- ```
  ...
  ```
   ...
   — The method called when an upstream module has finished work with a particular server. The
  ...
  This method also decrements the
